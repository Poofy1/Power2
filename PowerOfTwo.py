...
for i, filename in enumerate(os.listdir(start)):
    try: 
        imgCrop = Image.open(start + str(filename))
        imgCrop = imgCrop.convert('RGB')
        w, h = imgCrop.size
        if h > w:
            Dif = int((h - w)/2)
            imgCrop.crop((0, 0 + Dif, w, w + Dif)).save(final + str(i) + ".jpg")
        elif w > h:
            Dif = int((w - h)/2)
            imgCrop.crop((0 + Dif, 0, h + Dif, h)).save(final + str(i) + ".jpg")
        else:
            imgCrop.save(final + str(i) + ".jpg")

        img_array = cv2.imread(final + str(i) + ".jpg")
        new_array = cv2.resize(img_array, (resolution, resolution))
        cv2.imwrite(final + str(i) + '.jpg', new_array)
